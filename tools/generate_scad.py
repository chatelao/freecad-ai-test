...
"""
    ...
    filepath = os.path.join(output_dir, "benchy.scad")
    with open(filepath, "w") as f:
        f.write(scad_content)

    print(f"OpenSCAD file generated at {filepath}")

    # Try to render STL if openscad is available
    import subprocess
    try:
        stl_path = os.path.join(output_dir, "benchy_scad.stl")
        subprocess.run(["openscad", "-o", stl_path, filepath], check=True)
        print(f"OpenSCAD STL rendered at {stl_path}")
    except Exception as e:
        print(f"Could not render OpenSCAD STL: {e}")
